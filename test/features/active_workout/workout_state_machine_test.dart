...
void main() {
  group('WorkoutStateMachine', () {
    test('runs basic start -> move -> complete flow', () {
      final WorkoutStateMachine machine = WorkoutStateMachine(workout: _singleSetWorkout(loopCount: 1));
      machine.start();
    ...
    test('transitions through rest_between_loops when looping', () {
      final WorkoutStateMachine machine = WorkoutStateMachine(workout: _singleSetWorkout(loopCount: 2));
      machine.start();
      machine.startPrepNow();
    ...
    test('throws for invalid transition', () {
      final WorkoutStateMachine machine = WorkoutStateMachine(workout: _singleSetWorkout(loopCount: 1));
      expect(() => machine.completeMove(), throwsA(isA<InvalidTransitionException>()));
    });
...
Workout _singleSetWorkout({required int loopCount}) {
  return Workout(
    workoutId: 'w1',
    title: 'W1',
    sets: <WorkoutSet>[
